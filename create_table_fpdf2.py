...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.image('assets/pipingcircle.jpg', 15,6,18 )
        ...
        title_w = self.get_string_width(self.title)
        doc_w = self.w
        
        self.set_x((doc_w - title_w)/2)

        self.cell(10,10, self.title, border=False, new_y= 'NEXT', align='L')
        # line breack
    ...
    def footer(self):
        ...
        # 
        footer_text = f'PiCa_R0 (APP by Enginapps.it) - Page {self.page_no()}/{{nb}}'
        footer_w = self.get_string_width(footer_text)
        doc_w = self.w 
        #self.set_xy(doc_w/2,-15)
        self.set_xy((doc_w - footer_w)/2,-15)
        self.set_font('helvetica', 'I', 10)
        # page number
        self.cell(10, 10, f'PiCa_R0 (APP by Enginapps.it) - Page {self.page_no()}/{{nb}}')  
    ...
    def create_table(self, table_data, title='', data_size = 10, title_size=12, align_data='L', align_header='L', cell_width='even', x_start='x_default',emphasize_data=[], emphasize_style=None,emphasize_color=(0,0,0)): 
        ...
        title_size: 
                    the font size for the title of the table
        align_data: 
                    align table data
                    L = left align
                    C = center align
        ...
        self.set_font(style= 'B', size=title_size)   # font titolo tabella
        ...
        if  not isinstance(col_width, list):
            if x_start:
                self.set_x(x_start)
            for datum in header:
                #self.set_font('times', 'B', 10)  # font header
            ...
            for row in data:
                #self.set_font('times', '', 10)   #dati normali
